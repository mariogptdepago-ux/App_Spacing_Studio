...
/**
 * Aplicaciones incluidas en la instalación inicial.
 * Se copian UNA SOLA VEZ a la carpeta privada de Google Drive cuando
 * se abre el sistema por primera vez.
 *
 * Después de la instalación, los administradores pueden cargar nuevos
 * HTML directamente desde el panel; no necesitan volver a GitHub.
 */
const BUNDLED_APPS = [
  {
    id: 'numerico-ap4-eje1-d1-grado11',
    title: 'Razones relacionadas · Numérico AP4 · EJE1 · D1 · Grado 11',
    filename: 'Numerico-AP4-EJE1-D1-Grado11.html',
    resource: 'App_Numerico_AP4_EJE1_D1_Grado11'
  },
  {
    id: 'refuerzo-acumulativo-numerico-ap4-eje1-d1-grado11',
    title: 'Refuerzo acumulativo · Numérico AP4 · EJE1 · D1 · Grado 11',
    filename: 'RefuerzoAcumulativo-Numerico-AP4-EJE1-D1-Grado11.html',
    resource: 'App_RefuerzoAcumulativo_AP4_EJE1_D1_Grado11'
  }
];
...
function ensureBundledApps_() {
  // Marca persistente: si un Administrador elimina luego una app inicial,
  // no se recreará automáticamente en cada carga.
  if (getConfig_('BUNDLED_APPS_IMPORTED') === 'TRUE') return;

  const folder = getAppFolder_();
  const existing = new Set(getAllApps_().map(a => a.id));

  BUNDLED_APPS.forEach(def => {
    if (existing.has(def.id)) return;

    let html = '';
    try {
      html = HtmlService.createHtmlOutputFromFile(def.resource).getContent();
    } catch (err) {
      throw new Error(
        'Falta el archivo de aplicación inicial "' + def.resource +
        '.html" dentro del proyecto de Apps Script.'
      );
    }

    const file = folder.createFile(def.filename, html, MimeType.HTML);
    file.setDescription('Aplicación inicial privada administrada por ' + SYS.APP_NAME);
    ...
    appendRow_(SYS.SHEETS.APPS, [
      ...
      def.filename,
      file.getId(),
      false,       // Siempre nace DESACTIVADA.
      '',
      '',
      'SISTEMA',
      nowIso_(),
  ...
  setConfig_('BUNDLED_APPS_IMPORTED', 'TRUE');
}
